...
**交叉引用**：Stage 5 §3（3 种失败模式）；`project-governance` skill（Kanban-first 规则）。
...
**交叉引用**：Stage 11 §2.3 Tier 2（编码子 agent 强制）；`project-governance` Pitfall #15。
...
**交叉引用**：Stage 7 §2 G3；`project-governance` skill（两层 review 规则）。
...
**交叉引用**：Stage 11 §6.3（提升到框架级规则）；`project-governance` Pitfall #7。
...
**交叉引用**：Stage 6 §9（边界纪律）；`project-governance` Pitfall #8。
...
**交叉引用**：Stage 11 §6.1 E2；`project-governance` Pitfall #11。
...
**交叉引用**：Stage 11 §5.3（跨 profile 通信）；`project-governance` Pitfall #14。
...
**交叉引用**：Stage 11 §6.1 E6；`project-governance` Pitfall #12。
...
**交叉引用**：Stage 11 §6；`project-governance` Pitfall #17；`agent-team-orchestrator` v0.1.0 教训（your project docs 现在有 §12，不是"缺失"）。
...
| 来源 | 数量 | 风格 |
|------|------|------|
| `project-governance` skill | 18（那里编号 #1-#18） | 项目特定实例；本索引抽象它们 |
...
4. 如果 pitfall 来自项目特定源（如 your project），添加前先泛化（索引与项目无关）
...
> "我在用 project Kanban —— 哪些治理 pitfalls 适用？"

交叉引用 §4：`project-governance` skill pitfalls，然后按症状查 §3。
...
- [`~/.hermes/profiles/ezio-zero/skills/software-development/project-governance/`](https://github.com/Ezio0/Hermes-Governance) — 原始 项目特定 pitfalls（1-18）
