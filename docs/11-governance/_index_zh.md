...
> 项目特定实例（如 project Kanban board、patch 格式）在它们自己的 governance skills。
...
| 项目特定的 Kanban 配置 | 项目的 governance skill（如 `project-governance`） |
...
Handbook（本文）           → 框架：规则、角色、升级路径
                              与项目无关
project-governance skill  → 实例：your project 特定 Kanban board、patch 目录、pitfalls
                              与项目相关
...
- Skill 需要在所有 profiles 可用（如 `project-governance`）
...
一些 skill 需要在每个 profile（如 `project-governance`、`global-launch-review`）。
这些是**共享 skills**。
...
- 环境的稳定事实（如 "your project runs on port 8000"）
...
- [`~/.hermes/profiles/ezio-zero/skills/software-development/project-governance/`](https://github.com/Ezio0/Hermes-Governance) — 项目特定实例（project Kanban board、patch 格式、18 个 pitfalls）
